    ...
    const cursor = users
      .find(filter, {
        projection: {
          email: 1,
          role: 1,
          profile: { fullName: 1, phone: 1, dob: 1, address: 1, sex: 1 },
        },
      })
      .skip(skip)
      .limit(limit)
...
export async function POST(req: Request) {
  try {
    ...
    if (!fullName || !email || !password || !dob || !phone || !address) {
      return NextResponse.json(
        {
          error:
            "Full name, email, password, date of birth, phone, and address are required.",
        },
        { status: 400 }
      );
    }
